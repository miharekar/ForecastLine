...
class ForecastLineView extends Ui.View {
  ...
  function onUpdate(dc) {
    if (data_at == null || data_at != App.Storage.getValue(ForecastLine.DATA_AT)) {
      data_at = App.Storage.getValue(ForecastLine.DATA_AT);
      setColors();
      drawBackground(dc);
      data = dataForDisplay();
  ...
  function dataForDisplay() {
    ...
    var start = 0;
    var hourly = App.Storage.getValue(ForecastLine.HOURLY);
  ...
  function setColors() {
    var bg = App.Properties.getValue("background");
    var contrast = App.Properties.getValue("contrast");
  ...
  function display(dc) {
    ...
    var currently =  App.Storage.getValue(ForecastLine.CURRENTLY);
    if (currently == null) {
      drawRefreshing(dc);
    } else {
  ...
  function drawEmpty(dc) {
    ...
    var error = App.Storage.getValue(ForecastLine.ERROR);
    var text;
    if (coordinates == null) {
  ...
  function drawLocation(dc) {
    var text;
    if (App.Properties.getValue("coordinates")) {
      text = App.Storage.getValue(ForecastLine.LATITUDE).format("%.4f") + ", " + App.Storage.getValue(ForecastLine.LONGITUDE).format("%.4f");
    } else {
      text = App.Storage.getValue(ForecastLine.LOCATION);
    }
  ...
  function drawCurrent(dc, currently) {
    ...
    drawIcon(dc, x - 10 - spacing, y + 10, currently[ForecastLine.ICON]);
    drawTemperature(dc, x, y + (_screenSize[1]/10) - (Gfx.getFontHeight(Gfx.FONT_TINY)/4*3), currently[ForecastLine.TEMPERATURE]);
  }
